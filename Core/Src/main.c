...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
/* (HID_REPORT_* / USBD_HID_REQ_* defines 已移至 dap_app.c) */
/* USER CODE END PTD */
...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
/* (USB_Request/Response 缓冲区及相关变量已移至 dap_app.c) */
/* USER CODE END PM */
...
//}
/* (USBD_HID0_GetReport / SetReport / Initialize、USBD_InEvent / OutEvent、
   DAP_Test_HaltCPU 均已迁移到 app/Src/dap_app.c，此处不再重复定义) */
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	HAL_Delay(50);
	WS2812C_Init();        /* 初始化 RGB LED */
	/* Enable DWT cycle counter (供 DAP 使用) */
	CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;
	DWT->CYCCNT = 0;
	DWT->CTRL |= DWT_CTRL_CYCCNTENA_Msk;
	DAP_Setup();           /* CMSIS-DAP SWD 引脚初始化 */
	/* MX_USB_DEVICE_Init / Bridge_Init / USBD_HID0_Initialize / StartDAPTask
	   均在 FreeRTOS 的 StartDefaultTask 中完成，不在这里调用，
	   否则 FreeRTOS 永远无法启动！ */
  /* USER CODE END 2 */
