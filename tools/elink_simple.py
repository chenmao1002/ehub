...
try:
    ...
    print(f"DPIDR = 0x{d[0]:08X}")
    xfer(s, [(0,0,1,0x50000000)])
    time.sleep(0.1)
    cnt,ack,d = xfer(s, [(0,1,1,0)])
    print(f"CTRL/STAT = 0x{d[0]:08X}")
    ...
    print(f"Verify: cnt={cnt} data={[f'0x{v:08X}' for v in d]}")
    ...
    for wpb in [4, 8, 16]:
        ...
        for b in range(blocks):
            ...
            if cnt > 0:
                ...
                print(f"Block {b} fail: cnt={cnt} ack={ack}")
                break
        t1 = time.time()
        dt = t1 - t0
        print(f"WPB={wpb:2d}: {total}B / {dt:.3f}s = {total/dt/1024:.1f} KB/s ({total*8/dt/1000:.1f} kbit/s)")
