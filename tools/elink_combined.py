"""
elaphureLink 速度测试 - 合并 TAR+DRW 为单个 Transfer
"""
...
def read_words(s, addr, n):
    """Read n words: combined TAR write + DRW reads in one Transfer"""
    transfers = [(1, 0, 1, addr)] + [(1, 1, 3, 0)] * n
    cnt, ack, data = xfer(s, transfers)
    return data

def write_words(s, addr, values):
    """Write words: combined TAR write + DRW writes in one Transfer"""
    transfers = [(1, 0, 1, addr)] + [(1, 0, 3, v) for v in values]
    cnt, ack, data = xfer(s, transfers)
    return cnt

try:
    log("=== elaphureLink Speed Test (combined TAR+DRW) ===")
    ...
    # Verify: read first 4 words using combined TAR+DRW
    d = read_words(s, 0x08000000, 4)
    log(f"Verify: [{', '.join(f'0x{v:08X}' for v in d)}]")
    
    # Second read immediately
    d2 = read_words(s, 0x08000010, 4)
    log(f"Verify2: [{', '.join(f'0x{v:08X}' for v in d2)}]")
    
    # Third read
    d3 = read_words(s, 0x08000020, 4)
    log(f"Verify3: [{', '.join(f'0x{v:08X}' for v in d3)}]")
    
    # ==================== SPEED TESTS ====================
    log("\n=== READ SPEED (SWD 1MHz) ===")
    
    for wpb in [4, 8, 15]:  # 15 max to keep tar_write+reads in one transfer
        total_target = 4096
    ...
    for b in range(blocks):
        ...
        if data:
            ...
            log(f"  Block {b} fail"); break
    t1 = time.time()
    dt = t1 - t0
    if total_bytes > 0:
        log(f"  {total_bytes}B / {dt:.3f}s = {total_bytes/dt/1024:.1f} KB/s ({total_bytes*8/dt/1000:.1f} kbit/s)")
    
    # SRAM WRITE speed
    log("\n=== SRAM WRITE SPEED (WPB=15) ===")
    WPB = 15
    blocks = 4096 // (WPB * 4)
    total_bytes = 0
    time.sleep(0.3)
    ...
    for b in range(blocks):
        ...
        values = [(b*WPB+i) & 0xFFFFFFFF for i in range(WPB)]
        cnt = write_words(s, addr, values)
        if cnt > 0:
            total_bytes += WPB * 4
        else:
            log(f"  Block {b} fail"); break
